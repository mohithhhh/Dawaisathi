...
-- SECURITY: users.plan / explanation_count / subscription_end must only ever
-- change via the service-role client (payment verify, usage tracking in
-- app/api/explain). The RLS "Users can update own profile" policy only
-- checks row ownership (auth.uid() = id) — it does NOT restrict which
-- columns a user can set on their own row, so without this trigger any
-- signed-in user could call `supabase.from('users').update({ plan: 'paid' })`
-- from the browser and grant themselves a paid/subscription plan for free.
-- service_role (used by the admin client) bypasses this check.
